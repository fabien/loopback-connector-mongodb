...
MongoDB.prototype.collectionName = function(model, command, options) {
  var options = this.collectionOptions(model, command, options);
...
MongoDB.prototype.execute = function(model, command, options) {
  var collection = this.collection(model, command, options);
...
MongoDB.prototype.save = function(model, data, options, callback) {
  ...
  this.execute(model, 'save', options, data, { w: 1 }, function(err, result) {
    if (!err) {
...
MongoDB.prototype.buildWhere = function(model, where, options) {
  ...
  where = exports.sanitizeFilter(where, options);
...
MongoDB.prototype.buildSort = function(model, order, options) {
  ...
  if (order) {
    order = exports.sanitizeFilter(order, options);
    var keys = order;
...
MongoDB.prototype.replaceWithOptions = function(model, id, data, options, cb) {
  ...
  var idName = self.idName(model);
  delete data[idName];
  this.execute(model, 'update', options, { _id: id }, data, options, function(err, info) {
  ...
  if (self.db) {
    ...
    async.each(models, function(model, modelCallback) {
      ...
      async.each(indexList, function(index, indexCallback) {
        ...
        self.collection(model, 'autoupdate').createIndex(index.fields || index.keys, index.options, indexCallback);
      }, modelCallback);
    }, cb);
  } else {
    self.dataSource.once('connected', function() {
