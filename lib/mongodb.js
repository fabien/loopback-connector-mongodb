...
var TRANSACTIONS_SUPPORTED = false;
    ...
    new mongodb.MongoClient(self.settings.url, validOptions).connect(function(
      ...
      if (!err) {
        ...
        return urlParser(self.settings.url, self.settings, function(err, url) {
          ...
          self.supportsTransactions(function(e, supported) {
            TRANSACTIONS_SUPPORTED = Boolean(supported);
...
MongoDB.prototype.getDatabase = function(dbName, options) {
  return this.client.db(dbName, Object.assign({}, this.settings, options));
};
...
MongoDB.prototype.define = function(modelDefinition) {
  ...
  modelDefinition.model.getTransaction = function() {
    return self.getTransaction();
  };
  modelDefinition.model.transaction = function(models, fn) {
    if (arguments.length === 1 && typeof models === 'function') {
      return self.transaction(modelName, models);
    } else {
      models = [modelName].concat(models || []);
      return self.transaction(models, fn);
    }
  };
};
 ...
 */
MongoDB.prototype.collectionName = function(model, command, options) {
  options = this.collectionOptions(model, command, options);
  return options.collection || model;
};

/**
 * Get collection settings for a given model
 * @param {String} model Model name
 * @returns {Object} collection options
 */
MongoDB.prototype.collectionOptions = function(model, command, options) {
  var modelClass = this._models[model];
  if (modelClass && modelClass.settings && modelClass.settings.mongodb) {
    return modelClass.settings.mongodb;
  }
  return {};
};
 ...
 * @returns {*}
 */
MongoDB.prototype.collection = function(model, command, options) {
  if (!this.db) {
    throw new Error(g.f('{{MongoDB}} connection is not established'));
  }
  var collectionName = this.collectionName(model, command, options);
...
MongoDB.prototype.execute = function(model, command, options) {
  ...
  function doExecute() {
    ...
    try {
      collection = self.collection(model, command, options);
    } catch (err) {
      debug('Error: ', err);
      callback(err);
      return;
    ...
    self.notifyObserversAround(
      ...
      function(context, done) {
        ...
        withSession(model, command, options, args);
...
function withSession(model, command, options, args) {
  if (SESSION_COMMANDS.hasOwnProperty(command) &&
    typeof options === 'object' && typeof options.transaction === 'object' &&
    options.transaction instanceof Transaction && options.transaction.isInTransaction) {
    var appendOpts = SESSION_COMMANDS[command];
    if (appendOpts) {
      args.splice(args.length - 2, 0, {session: options.transaction.session});
    } else if (typeof args[args.length - 2] === 'object') {
      Object.assign(args[args.length - 2], {session: options.transaction.session});
    }
  }
};
...
MongoDB.prototype.buildWhere = function(model, where, options) {
  ...
  Object.keys(where).forEach(function(k) {
    ...
    if (k === 'and' || k === 'or' || k === 'not' || k === 'nor') {
      ...
      delete query[k];
      return;
    } else if (k === 'expr' && typeof cond === 'object') {
      options = Object.assign({}, options, {isExpr: true});
      query['$' + k] = self.buildWhere(model, {__cond: cond}, options).__cond;
      delete query[k];
      return;
    }
    ...
    if (spec) {
      if (spec === 'between') {
        ...
        var c = {__key: cond};
        query[k] = {$not: self.buildWhere(model, c, options).__key};
      } else if (spec.indexOf('$') === 0 && options && options.isExpr) {
        query[k] = {};
        query[k][spec] = cond;
      } else {
        query[k] = {};
...
function buildNearFilter(query, params) {
  ...
  params.forEach(function(near) {
    ...
    var property;
    var mongoKey = near.mongoKey || near.key;

    if (mongoKey) {
      // if mongoKey is an Array, set the $near query at the right depth, following the Array
      if (Array.isArray(mongoKey)) {
        property = query.where;

        for (var i = 0; i < mongoKey.length; i++) {
          var subKey = mongoKey[i];

          if (mongoKey.hasOwnProperty(i + 1)) {
            if (!property.hasOwnProperty(subKey)) {
              property[subKey] = Number.isInteger(mongoKey[i + 1]) ?
                [] :
                {};
            }
        ...
        property[mongoKey[i - 1]] = queryValue;
      } else {
        // mongoKey is a single string, just set it directly
        property = query.where[mongoKey] = queryValue;
      }
    }
  });
}
  ...
  if (self.db) {
    ...
    async.each(
      ...
      function(model, modelCallback) {
        var collectionOptions = Object.assign({}, self.collectionOptions(model, 'autoupdate'));
        ...
        // Don't create index for MongoDB View
        if (collectionOptions.viewOn || settings.disableAutoUpdate) {
          ...
          if (properties[p].index) {
            ...
            if (typeof properties[p].index === 'object') {
              ...
              if (typeof properties[p].index.mongodb === 'object') {
                options = Object.assign({}, properties[p].index.mongodb);
  ...
  if (self.db) {
    ...
    async.eachSeries(
      ...
      function(model, modelCallback) {
        ...
        var collectionOptions = Object.assign({}, self.collectionOptions(model, 'automigrate'));
        delete collectionOptions.collection; // skip
        ...
        self.db.dropCollection(collectionName, function(err, collection) {
          ...
          self.db.createCollection(collectionName, collectionOptions, modelCallback);
        });
      },
      function(err) {
        if (err || self.dataSource.settings.disableAutoUpdate) {
          return cb && cb(err);
        }
        self.autoupdate(models, cb);
      }
...
MongoDB.prototype.supportsTransactions = function(check) {
  if (check) {
    ...
    adminDb.serverStatus(function(err, info) {
      if (err) {
        ...
        check(err, info.version.indexOf('4.') === 0 &&
          typeof info.repl === 'object' &&
          Array.isArray(info.repl.hosts));
      }
    });
  }
  return this.client && TRANSACTIONS_SUPPORTED;
};

function wrapModel(modelBuilder, Model, fn) {
  var Copy = modelBuilder.copyModel(Model);
  Copy._observers = {};
  Copy._observers['before save'] = [].concat(Model._observers['before save'] || []);
  Copy._observers['before save'].unshift(fn);
  Copy._observers['before delete'] = [].concat(Model._observers['before delete'] || []);
  Copy._observers['before delete'].unshift(fn);
  return Copy;
};

MongoDB.prototype.transaction = function(models, fn) {
  var self = this;
  var modelBuilder = this.dataSource.modelBuilder;

  return new Promise(function(resolve, reject) {
    // always resolve (not reject)
    var transaction = new Transaction(self, resolve, resolve);

    var observeFn = function(ctx, next) {
      ctx.options = ctx.options || {};
      ctx.options.transaction = ctx.options.transaction || transaction;
      next();
    };

    var _models = [].concat(models || []).map(function(modelName) {
      var Model = modelBuilder.getModel(modelName);
      if (Model) return wrapModel(modelBuilder, Model, observeFn);
    });

    transaction.start();

    fn.apply(null, [transaction].concat(_models));
  });
};

MongoDB.prototype.getTransaction = MongoDB.prototype.startTransaction = function(fn) {
  if (this.supportsTransactions()) {
    ...
    throw new Error('MongoDB instance does not support transactions');
  }
};

function Transaction(connector, onCommit, onRollback) {
  this.connector = connector;
  this.isSupported = connector.supportsTransactions();
  this.isInTransaction = false;
  this.onCommit = onCommit || function() {};
  this.onRollback = onRollback || function() {};
};

Transaction.prototype.start = function(options, fn) {
  if (typeof options === 'function') {
    fn = options;
    options = null;
  }
  options = Object.assign({}, options);
  if (!this.isInTransaction && this.isSupported) {
    this.session = this.connector.client.startSession();
    this.session.startTransaction(options);
    this.isInTransaction = true;
  }
  if (typeof fn === 'function') {
    const self = this;
    return Promise.resolve(fn(this)).then(function() {
      return self.commit();
    }).catch(function() {
      return self.rollback();
    });
  } else {
    return Promise.resolve(this.isInTransaction);
  }
};

Transaction.prototype.handle = function(cb) {
...
Transaction.prototype.commit = function(cb) {
  if (this.isInTransaction) {
    return endTransaction(this, this.session.commitTransaction(), cb).then(this.onCommit).then(function() {
      return true;
    });
  } else {
    const p = Promise.resolve(typeof cb === 'function' ? cb() : function() {});
    return endTransaction(this, p).then(this.onCommit).then(function() {
      return false;
    });
  }
};
...
Transaction.prototype.rollback = function(cb) {
  const self = this;
  const onRollback = function() { return self.onRollback(); };
  if (this.isInTransaction) {
    return endTransaction(this, this.session.abortTransaction(), cb).then(onRollback).then(function() {
      return true;
    });
  } else {
    const p = Promise.resolve(typeof cb === 'function' ? cb() : function() {});
    return endTransaction(this, p).then(onRollback).then(function() {
      return false;
    });
  }
};
...
function endTransaction(self, promise, cb) {
  return Promise.resolve(promise).then(function() {
    if (self.session) self.session.endSession();
...
function optimizedFindOrCreate(model, filter, data, options, callback) {
  ...
  this.collection(model, 'findOneAndUpdate', options).findOneAndUpdate(
    query,
    {$setOnInsert: data},
